...
class AOS {
    ...
    handle(entries) {
        entries.forEach(entry => {
            entry.target.classList.toggle('_aos-done', entry.isIntersecting);
        });
    }
}
...
class GoTop {
    ...
    init() {
        window.addEventListener("scroll", () => {
            this.btn.classList.toggle("hidden", window.scrollY === 0);
        });
...
class Modal {
    ...
    show() {
        this.body.add('_overflowhidden');
        this.modal.classList.add('_show-modal');
    }
    hide() {
        this.modal.classList.remove('_show-modal');
        this.body.remove('_overflowhidden');
    }
    initListeners() {
...
class AdBlockTester {
    ...
    showInfo(span) {
        const testDiv = span.parentElement;
        document.querySelectorAll(".test").forEach(el => {
            if (el !== testDiv) el.classList.remove("show");
        });
        testDiv.classList.toggle("show");
    }

    // Concurrency limiter (prevents browser from freezing)
    ...
    async fetchTests() {
        ...
        for (const element in data) {
            ...
            for (const key in category) {
                ...
                div.innerHTML = `<span onclick="adBlockTester.showInfo(this)">${icons[key]}&nbsp;&nbsp;${key}</span>`;
                div.appendChild(dw);
                ...
                for (const u of items) {
                    // We will run these with concurrency later
        ...
        // Run all checks with concurrency control
        await this.withConcurrencyLimit(this.pendingChecks, 8);
    }
}

// Global instance so onclick in HTML still works
let adBlockTester;
...
window.onload = function () {
    ...
    adBlockTester.pendingChecks = [];   // will be filled in fetchTests
